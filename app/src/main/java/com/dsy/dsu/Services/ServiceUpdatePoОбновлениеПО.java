...
/////TODO  УДАЛЕНИЕ .JSON ФАЙЛА
            ...
            public void run() {
                ...
                try {
                    ...
                    КнопкаЗАгрузкиПО.setOnClickListener(new View.OnClickListener() {
                            ...
                            try {
                                ...
                                Maybe.fromAction(new Action() {
                                    ...
                                    public void run() throws Throwable {
                                        ...
                                        методАнализJSONВерсииПО(СервернаяВерсияПОВнутри);

                                        Log.w(getApplicationContext().getClass().getName(), Thread.currentThread().getStackTrace()[2].getMethodName() +
                                                " ЛокальнаяВерсияПО " + ЛокальнаяВерсияПО + " СервернаяВерсияПОВнутри  "
                        ...
                        private void методПослеЗаргузкиUpdatePO() {
                            try{
    ...
    private File методАнализJSONВерсииПО(@NonNull Integer СервернаяВерсияПОВнутри ) {
        try{
